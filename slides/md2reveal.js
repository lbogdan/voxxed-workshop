...
function mdRender(mdContent) {
  ...
  result = result.replace(/class="language-plaintext"/g, 'class="plaintext"');
  result = result.replace(/<img /g, '<img style="width: 400px" ');
  return result;
}
...
const revealSlides = slides.map(slide => {
  ...
  <img
    ...
    alt="JSLeague Logo"
  />
  <div>One Application, 3 Frameworks :: Vue.js</div>
</div>
${mdRender(slide)}
</section>
`;
